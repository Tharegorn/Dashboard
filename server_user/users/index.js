...
exports.refreshToken = async function (req, res) {
    ...
    jwt.verify(token, process.env.REFRESH_TOKEN_SECRET, async function (err, user) {
        if (err) res.status(401).json({ status: "Unauthorized, invalid Token", code: 401 });
            ...
            if (rest[0]) {
                const resp = await conn.query("SELECT id, name FROM users WHERE id='" + rest[0].user_id + "';")
                const token = generateToken({ id: resp[0].id, name: resp[0].name });
                const refresh = generateRefreshToken({ id: resp[0].id, name: resp[0].name });
                conn.query("UPDATE tokens SET token='" + token + "', refresh='" + refresh + "' WHERE user_id='" + resp[0].id + "';")
                res.status(200).json({ status: "Success", code: 200, token: token, refresh: refresh });
            } else
...
exports.login = async function (req, res) {
    let conn = await pool.getConnection();
    const resp = await conn.query("SELECT id, name, password FROM users WHERE name='" + req.body.user + "' LIMIT 1;");
    if (resp[0]) {
...
exports.loadUsers = async function (req, res) {
    ...
    const resp = await conn.query("SELECT id, name, perm, added_at AS date FROM users WHERE name !=' " + req.user.name + "';")
